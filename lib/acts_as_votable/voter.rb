  ...
  module Voter
    ...
    def self.included(base)
      ...
      base.class_eval do
        ...
        has_many :voter_votes, :class_name => "ActsAsVotable::Vote", :as => :voter do
          def votables
    ...
    def vote_up_for model=nil, args={}
      voter_vote :votable => model, :vote_scope => args[:vote_scope], :vote => true
    end

    def vote_down_for model=nil, args={}
      voter_vote :votable => model, :vote_scope => args[:vote_scope], :vote => false
    end
    ...
    def voted_on? votable, args={}
      voter_votes = find_voter_votes(:votable_id => votable.id, :votable_type => votable.class.name,
    ...
    def voted_as_when_voting_on votable, args={}
      voter_votes = find_voter_votes(:votable_id => votable.id, :votable_type => votable.class.name,
                         :vote_scope => args[:vote_scope])
      return nil if voter_votes.size == 0
      return voter_votes.first.vote_flag
    end
    alias :voted_as_when_voted_for :voted_as_when_voting_on

    def find_voter_votes extra_conditions = {}
      voter_votes.where(extra_conditions)
    end

    def find_up_votes args={}
      find_voter_votes :vote_flag => true, :vote_scope => args[:vote_scope]
    end

    def find_down_votes args={}
      find_voter_votes :vote_flag => false, :vote_scope => args[:vote_scope]
    end

    def find_voter_votes_for_class klass, extra_conditions = {}
      find_voter_votes extra_conditions.merge({:votable_type => klass.name})
    end

    def find_up_votes_for_class klass, args={}
      find_voter_votes_for_class klass, :vote_flag => true, :vote_scope => args[:vote_scope]
    end

    def find_down_votes_for_class klass, args={}
      find_voter_votes_for_class klass, :vote_flag => false, :vote_scope => args[:vote_scope]
    end
    ...
    def get_voted klass, extra_conditions = {}
      klass.joins(:voter_votes).merge find_voter_votes(extra_conditions)
    end

    def get_up_voted klass
      klass.joins(:voter_votes).merge find_up_votes
    end

    def get_down_voted klass
      klass.joins(:voter_votes).merge find_down_votes
    end
  end
end
